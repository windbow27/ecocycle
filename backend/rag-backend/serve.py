...
def handle_query():
    ...
    if guidedRoute == PRODUCT_ROUTE_NAME:
        ...
        combined_information = f"Hãy trở thành một giáo viên thân thiện và dễ hiểu, chuyên tư vấn các kiến thức về tái chế và bảo vệ môi trường cho các em học sinh trong lớp học. Khi có câu hỏi từ học sinh: {query}, bạn hãy trả lời dựa trên các thông tin dưới đây và nhớ chèn các icon thích hợp để làm nổi bật nội dung quan trọng hoặc tạo sự gần gũi: {source_information}."
        data.append({
            "role": "user",
            "parts": [
                {
